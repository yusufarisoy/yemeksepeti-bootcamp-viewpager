...
class MainOnBoardingFragment: BaseFragment() {
    ...
    private fun initListeners() {
        ...
        skipButton.setOnClickListener {
            if(viewPager.currentItem + 1 < 3) {
                ...
                findNavController().navigate(R.id.action_mainOnBoardingFragment_to_homeFragment)
            }
        }
    }
    ...
    private fun initIndicators() {
        ...
        for (i in indicators.indices) {
            ...
            if(i == 0) {
                indicators[i]?.background = ResourcesCompat.getDrawable(resources, R.drawable.indicator_active, null)
                indicators[i]?.layoutParams = layoutParamsFirst
            } else {
                indicators[i]?.background = ResourcesCompat.getDrawable(resources, R.drawable.indicator_inactive, null)
    ...
    private fun setSelectedIndicator(position: Int) {
        for(i in 0 until viewPagerIndicator.childCount) {
            ...
            if(i == position) {
                val layoutParamsSelected: LinearLayout.LayoutParams = LinearLayout.LayoutParams(48, 48)
                layoutParamsSelected.marginEnd = 16
                indicator.layoutParams = layoutParamsSelected
                indicator.background = ResourcesCompat.getDrawable(resources, R.drawable.indicator_active, null)
            } else {
                val layoutParamsInactive: LinearLayout.LayoutParams = LinearLayout.LayoutParams(30, 30)
                layoutParamsInactive.marginEnd = 16
                indicator.layoutParams = layoutParamsInactive
                indicator.background = ResourcesCompat.getDrawable(resources, R.drawable.indicator_inactive, null)
            }
        }
    }
}